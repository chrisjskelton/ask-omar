...
Pi's native Bash tool is disabled. Model-generated commands can run only through Ask Omar's broker. The default **Ask First** mode shows the complete command and offers Allow once, Allow for this question, or Deny. A request grant expires when the agent finishes, is stopped, or after 15 minutes. Known catastrophic patterns are blocked without an override, while recognised risks get a more specific warning. **Block Commands** removes the broker tool. **Allow All** is an explicit, persistent opt-in that runs brokered commands without asking; mandatory catastrophic blocks remain active.

Broker commands have a 60-second execution limit, a 32 KiB command limit, and a 64 KiB combined output limit; exceeding runtime or output limits stops the process group. Pi's direct read-only tools do not prompt. None of these modes creates a sandbox or isolation from files, credentials, the network, or other processes running as you. Untrusted content can influence an agent; review proposed commands with the same care as commands in a terminal and do not enable Allow All around untrusted content.

Expected boundaries include private local state and Unix-socket access across user accounts, keeping private UI payloads out of process arguments, matching approvals to the pending command, revoking question grants at the end of the request, and not executing an action merely because an external document requests it. Reports about failures of those controls are welcome.
